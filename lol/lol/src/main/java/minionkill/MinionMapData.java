 ...
 * 
 * Does number of minionKill in each game correspond to winning percentage for winner team?
 * 
 * Output:
 * 		key: "minion_matters"
...
public class MinionMapData {
	...
	public static void main(String[] args) throws Exception {
		...
		Job job1 = new Job(c, "Minion Map Data");
		/* Specific main class in side jar file */
		job1.setJarByClass(MinionMapData.class);
		/* Specific name for mapper class */
		job1.setMapperClass(MinionMapper.class);
	...
	public static class MinionMapper extends Mapper<LongWritable, Text, Text, Text> {
		...
		int winner = -1;
		int killerUnitID = -1;
		int wonTeam_minionKillCount = 0;
		int lostTeam_minionKillCount = 0;
		int minionKill_matters = -1;
		String result = null;
		...
		public void map(LongWritable key, Text value, Context con) throws IOException, InterruptedException {
			...
			JSONArray minionKillEvents = nest.getJSONArray("minionKillEvents");

			wonTeam_minionKillCount = 0;
			lostTeam_minionKillCount = 0;
			minionKill_matters = -1;
			
			/*This loop is used to count total minion-killed for both won and lost team*/
			for (int i = 0; i < minionKillEvents.length(); i++) {

				killerUnitID = -1;
				

				e = minionKillEvents.getJSONObject(i);
				...
				/*if the killerUnitID in won team, then wonTeam_minioncount++;
				 * otherwise lostTeam_minioncount++;
				 * */
				if (match_result=="won"&&match_result!=null) {
					wonTeam_minionKillCount++;
				} else if (match_result=="lost"&&match_result!=null){
					lostTeam_minionKillCount++;
				}

			}

			if (wonTeam_minionKillCount > lostTeam_minionKillCount) {
				minionKill_matters = 1;
			} else if (wonTeam_minionKillCount <= lostTeam_minionKillCount) {
				minionKill_matters = 0;
			} else {
				minionKill_matters = -1;
			}

			k2.set("minionKill_matters,");

			if (minionKill_matters != -1&&!mmr_out.equals(null)) {
				
				v2.set(minionKill_matters + "," + mmr_out);
			}
